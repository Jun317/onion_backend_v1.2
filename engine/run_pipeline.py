...
from . import cluster, dedup, express, importance, lifecycle
...
from .export import export_all, export_debug
...
def run(dry_run: bool = False, db_path: str | None = None,
        ...
        lookback_h: int | None = None) -> dict:
    conn = connect(db_path)
    data = data_dir or DATA
    stats: dict = {}
    try:
        # 1) 수집 산출 반영
        records = load_recent_raw(data, lookback_h=lookback_h)
        ...
        stats["importance"] = importance.recompute_all(conn)
        ...
        stats["export_debug"] = export_debug(conn, out_dir, current_stats=stats)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--lookback-h", type=int, default=None,
                    help="raw 재읽기 창(시간) 오버라이드 — 백필 직후 수동 실행용")
    ...
    if args.dry_run:
        ...
        run(lookback_h=args.lookback_h)
    return 0
